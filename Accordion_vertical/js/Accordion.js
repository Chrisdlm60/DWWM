...
var vaccordion={
	...
	expandli:function(accordionid, targetli){
		var config=vaccordion.accordioninfo[accordionid]
	...
	init:function($, config){
			vaccordion.accordioninfo[config.accordionid]=config //cache config info for this accordion
			...
			$targetlis.each(function(i){
				...
				$target[vaccordion.ismobile? "click" : "mouseenter"](function(){
						vaccordion.expandli(config.accordionid, this)
	...
	uninit:function($, config){
		...
		if (config.selectedli[1]==true) //enable persistence?
			vaccordion.setCookie(config.accordionid, expandedliindex)
	},
	...
	setup:function(config){
		...
		jQuery(document).ready(function($){ //on Dom load
			if (config.ajaxsource) //if config.ajaxsource option defined
				vaccordion.loadexternal($, config)
			else
				vaccordion.init($, config)
		}) //end DOM load
	}
