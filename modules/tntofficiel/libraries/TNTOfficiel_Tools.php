...
class TNTOfficiel_Tools
{
    ...
    {
        // Get type.
        $strType = gettype($args);
        if (is_object($args)) {
            $strType = '('.$strType.')'.get_class($args);
        } elseif (is_resource($args)) {
            $strType = '('.$strType.')'.get_resource_type($args);
        }
     ...
     * @return array
     */
    public static function dumpSafe($mxdArgValue, $intArgMemLimit = 1048576)
    {
        if (is_object($mxdArgValue) || is_array($mxdArgValue)) {
            try {
                $arrCallerArgsDeep = array();
                $intPropCount = max(count((array)$mxdArgValue), 1);
                foreach ($mxdArgValue as $k => $mxdTraceArg) {
                    $arrCallerArgsDeep[$k] = array(
                        'Type' => TNTOfficiel_Tools::getType($mxdTraceArg),
                        'Value' => (TNTOfficiel_Tools::getMem($mxdTraceArg) > ($intArgMemLimit / $intPropCount)) ?
                            null : TNTOfficiel_Tools::getSafe($mxdTraceArg)
                    );
                }
                $mxdArgValue = $arrCallerArgsDeep;
            } catch (Exception $objException) {
                $mxdArgValue = TNTOfficiel_Tools::getSafe($mxdArgValue);
            }
        } else {
            $mxdArgValue = TNTOfficiel_Tools::getSafe($mxdArgValue);
        }
        ...
        return array(
            'Type' => TNTOfficiel_Tools::getType($mxdArgValue),
            'Value' => (TNTOfficiel_Tools::getMem($mxdArgValue) > $intArgMemLimit) ? null : $mxdArgValue
        );
    }
    ...
    public static function encJSON($mxdArgValue, $intArgPettyLevel = 3)
    {
        ...
        if ($intArgPettyLevel > 0 && $intArgPettyLevel <= 2) {
            // tab to 2 spaces.
            $strJSON = preg_replace('/\ {4}/ui', '  ', $strJSON);
            // before }
...
SQL;
     ...
     * @return bool|null
     */
    public static function isTodayOrTomorrow($mxdArgDateTime)
    {
        $objDateTime = TNTOfficiel_Tools::getDateTime($mxdArgDateTime);
        if ($objDateTime === null) {
            return null;
