...
SQL;
    ...
    public function setMedia($isNewTheme = false)
    {
        ...
        $this->context->controller->addJS($strAssetJSPath.'AdminCarrierSetting.js');
    }
            ...
            if (count($arrFormMessagesServiceTnt['error']) === 0) {
                ...
                if (is_array($arrCarrierCreated)) {
                    Tools::redirectAdmin(
                        ...
                        .'&updatecarrier&init_carrier='.urlencode(serialize($arrCarrierCreated))
...
*/
            ...
            if ($strArgSpecificPrice) {
                foreach ($arrZonesConfigPost as $intZoneConfID => &$arrZoneConf) {
                    ...
                    if (!array_key_exists($strZoneKeyLabel, $arrFormMessagesZones['error'])
                        ...
                        $arrInitCarrier = unserialize(Tools::getValue('init_carrier'));
                        // Add
                        if ($arrInitCarrier) {
        ...
        $arrCarrierID = unserialize(Tools::getValue('init_carrier'));
        if (!$arrCarrierID) {
            $arrCarrierID = array(Tools::getValue('id_carrier'));
        }
        $arrServicesName = array();
        if ($arrCarrierID) {
            foreach ($arrCarrierID as $intCarrierID) {
                $objTNTCarrierModel = TNTOfficielCarrier::loadCarrierID($intCarrierID, false);
                $strCurrentServiceLabel = $objTNTCarrierModel->getCarrierInfos()->label;
                if (!array_key_exists($strCurrentServiceLabel, $arrServicesName)) {
                    $arrServicesName[$strCurrentServiceLabel] = array();
                }
                $arrServicesName[$strCurrentServiceLabel][] = $objTNTCarrierModel->getPSShop()->name;
            }
        }

        $strServicesName = '';
        foreach ($arrServicesName as $strN => $arrS) {
            $strServicesName .= '<li>'.sprintf(
                $this->l('%s on %s'),
                '<b>'.$strN.'</b>',
                '<b>'.implode('</b>, <b>', $arrS).'</b>'
            ).'</li>';
        }
        $strServicesName = '<ul>'.$strServicesName.'</ul>';
            ...
            'input' => array(
                ...
                array(
                    ...
                    'html_content' => '<div class="current-service">'
                        .$this->l('TNT service being modified').' :'.$strServicesName
                        .'</div>'.'<br />'
                ),
                array(
                    'type' => 'switch',
    ...
    public function renderForm()
    {
        ...
        $arrFormStruct[$strIdFormPriceZone] = array('form' => array(
            'legend' => array(
