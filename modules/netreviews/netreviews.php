 ...
 * @version   Release: $Revision: 7.9.4
 * @date      30/01/2019
...
class NetReviews extends Module
{
    ...
    public function getContent()
    {
        ...
        if (_PS_VERSION_ >= '1.5') {
            ...
            if ($isMultishop && Shop::getTotalShops() > 1 && _PS_VERSION_ >= '1.5' /*&& _PS_VERSION_ < '1.7'*/ && (Configuration::hasKey(
                    'AV_IDWEBSITE',
                    null,
                    null,
                    null
                ...
                Configuration::updateValue('AV_IDWEBSITE', $idWebsiteShop1, null, $this->current_shop_id);
                Configuration::deleteByName('AV_CLESECRETE');
                Configuration::updateValue('AV_CLESECRETE', $secretKeyShop1, null, $this->current_shop_id);
                Configuration::deleteByName('AV_GOUPINFO');
                Configuration::updateValue('AV_GOUPINFO', $groupInfoShop1, null, $this->current_shop_id);
                Configuration::deleteByName('AV_MULTISITE');
                Configuration::updateValue('AV_MULTISITE', $multisite, null, $this->current_shop_id);
        ...
        // update av_groupinfo
        Configuration::updateValue('AV_GOUPINFO', json_encode($current_avisverifies_idwebsite), false, null, $this->current_shop_id);
            ...
            if($value_keycheck){
                foreach ($value_keycheck as $lang_check => $value) {
                    if ($lang_check != "root" && $value!=""){
                        $keycheck_result["lan"][] = $value;
                    }elseif($lang_check == "root" && $value!=""){
                        $keycheck_result["root"][] = $value;
                    }
                }
            }
        }

        $double_key_found = false;
        if(isset($keycheck_result["lan"]) && (count($keycheck_result["lan"]) !== count(array_unique($keycheck_result["lan"])))){
            $double_key_found = true;
        } elseif (isset($keycheck_result["root"]) && (count($keycheck_result["root"]) !== count(array_unique($keycheck_result["root"])))) {
            $double_key_found = true;
        }

        if (!empty($keycheck_result) && $double_key_found){
    ...
    {
        if($idShop == null && _PS_VERSION_ < '1.7' && _PS_VERSION_ >= '1.6') {
            $idShop = 1;
        }
