...
class ContactFormDevis extends Module implements WidgetInterface
{
    /** @var string */
    const SEND_CONFIRMATION_EMAIL_DEVIS = 'CONTACTFORM_SEND_CONFIRMATION_EMAIL_DEVIS';

    /** @var string */
    const SEND_NOTIFICATION_EMAIL_DEVIS = 'CONTACTFORM_SEND_NOTIFICATION_EMAIL_DEVIS';
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->trans('Contact devis form', [], 'Modules.ContactFormDevis.Admin');
        $this->description = $this->trans(
            'Adds a contact devis form to the "Devis" page.',
            [],
            'Modules.ContactFormDevis.Admin'
        );
    ...
    public function getContent()
    {
        ...
        if (Tools::getValue(self::SUBMIT_NAME)) {
            Configuration::updateValue(
                self::SEND_CONFIRMATION_EMAIL_DEVIS,
                Tools::getValue(self::SEND_CONFIRMATION_EMAIL_DEVIS)
            );
            Configuration::updateValue(
                self::SEND_NOTIFICATION_EMAIL_DEVIS,
                Tools::getValue(self::SEND_NOTIFICATION_EMAIL_DEVIS)
            );
        }
    ...
    {
        $fieldsValue = [
            self::SEND_CONFIRMATION_EMAIL_DEVIS => Tools::getValue(
                self::SEND_CONFIRMATION_EMAIL_DEVIS,
                Configuration::get(self::SEND_CONFIRMATION_EMAIL_DEVIS)
            ),
            self::SEND_NOTIFICATION_EMAIL_DEVIS => Tools::getValue(
                self::SEND_NOTIFICATION_EMAIL_DEVIS,
                Configuration::get(self::SEND_NOTIFICATION_EMAIL_DEVIS)
            )
        ];
        $form = [
            'form' => [
                'legend' => [
                    'title' => $this->trans('Parameters', [], 'Modules.ContactFormDevis.Admin'),
                    'icon' => 'icon-envelope'
                ],
                'input' => [
                    [
                        ...
                        'name' => self::SEND_CONFIRMATION_EMAIL_DEVIS,
                        'is_bool' => true,
                        'required' => true,
                        'values' => [
                            [
                                'id' => self::SEND_CONFIRMATION_EMAIL_DEVIS . '_on',
                                'value' => 1,
                                'label' => $this->trans('Enabled', [], 'Admin.Global')
                            ],
                            [
                                'id' => self::SEND_CONFIRMATION_EMAIL_DEVIS . '_off',
                                'value' => 0,
                    ...
                    [
                        ...
                        'name' => self::SEND_NOTIFICATION_EMAIL_DEVIS,
                        'is_bool' => true,
                        'required' => true,
                        'values' => [
                            [
                                'id' => self::SEND_NOTIFICATION_EMAIL_DEVIS . '_on',
                                'value' => 1,
                                'label' => $this->trans('Enabled', [], 'Admin.Global')
                            ],
                            [
                                'id' => self::SEND_NOTIFICATION_EMAIL_DEVIS . '_off',
                                'value' => 0,
                                'label' => $this->trans('Disabled', [], 'Admin.Global')
                            ]
                        ]
    ...
    public function renderWidget($hookName = null, array $configuration = [])
    {
        ...
        return $this->display(__FILE__, 'views/templates/widget/contact_devis.tpl');
    }
    ...
    public function getWidgetVariables($hookName = null, array $configuration = [])
    {
        ...
        $this->contact['societe'] = Tools::getValue('societe');
        ...
        if (!($from = trim(Tools::getValue('from'))) || !Validate::isEmail($from)) {
            ...
            $this->context->controller->errors[] = $this->trans(
                ...
                'Shop.Notifications.Error'
                  ...
                  !(Validate::isLoadedObject($contact_devis = new Contact($id_contact, $this->context->language->id)))
        ) {
            $this->context->controller->errors[] = $this->trans(
                'Please select a subject from the list provided. ',
                [],
                'Modules.ContactFormDevis.Shop'
            );
            ...
            if ($contact_devis->customer_service) {
            ...
            $sendConfirmationEmail = Configuration::get(self::SEND_CONFIRMATION_EMAIL_DEVIS);
            $sendNotificationEmail = Configuration::get(self::SEND_NOTIFICATION_EMAIL_DEVIS);
            ...
            if (!count($this->context->controller->errors)
                ...
                $var_list = [
                    ...
                    '{email}' =>  $from,
                    '{product_name}' => '',
                    '{societe}' => Tools::nl2br(Tools::htmlentitiesUTF8(Tools::stripslashes($societe))),
                ];
                ...
                if ($sendNotificationEmail) {
                    if (empty($contact_devis->email) || !Mail::Send(
                        $this->context->language->id,
                        'contact',
                        $this->trans('Message from contact form devis', [], 'Emails.Subject').' [no_sync]',
                        $var_list,
                        $contact_devis->email,
                        $contact_devis->name,
                        null,
